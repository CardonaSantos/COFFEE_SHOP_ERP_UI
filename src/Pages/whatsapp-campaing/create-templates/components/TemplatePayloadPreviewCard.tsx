...
export function TemplatePayloadPreviewCard({
  ...
  return (
    <div className="space-y-3">
      ...
      {submitResult && (
        <Alert className="border-green-200 bg-green-50 dark:border-green-900 dark:bg-green-950/30">
          ...
          <AlertDescription className="text-xs text-green-700 dark:text-green-400">
            <span className="font-medium">Plantilla enviada a revisión correctamente.</span>
            <div className="mt-1 flex flex-wrap gap-1.5">
              <span className="text-muted-foreground">ID:</span>
              <code className="rounded bg-muted px-1 py-0.5 text-[10px]">{submitResult.id}</code>
      ...
      <Card>
        <CardHeader className="pb-2 pt-4 px-4">
          <div className="flex items-center justify-between">
            <CardTitle className="text-sm font-medium">Preview del payload</CardTitle>
            <Button
              variant="ghost"
              size="icon"
              className="size-7"
          ...
          {json ? (
            ...
            <div className="flex items-center gap-2 rounded-md border bg-muted/30 px-3 py-4">
              ...
              <p className="text-xs text-muted-foreground">
                Completa el formulario para ver el payload que se enviará a Meta.
              </p>
            </div>
          )}
        </CardContent>
      ...
      {/* Advanced templates note */}
      <div className="flex gap-2 rounded-md border border-dashed bg-muted/20 px-3 py-2.5">
        <AlertCircle className="size-3.5 mt-0.5 shrink-0 text-muted-foreground" />
        <p className="text-xs text-muted-foreground leading-relaxed">
          Las plantillas avanzadas con carousel o estructuras complejas se crearán con{" "}
          <code className="rounded bg-muted px-1 py-0.5 text-[10px]">
            POST /whatsapp-template/raw
          </code>
          . Esta pantalla cubre plantillas estándar.
        </p>
      </div>
    </div>
  )
